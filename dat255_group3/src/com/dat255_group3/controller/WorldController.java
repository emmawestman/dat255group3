...
public class WorldController {
	...
	public void uppdatePositions(Body body, Character character) {
		Vector2 posInPixels = CoordinateConverter.meterToPixel(body
				.getPosition());
	...
	/**
	 * Make the physical character move with the speed (meter/sec) of the
	 * camera. If the character is too far ahead of the camera the character
	 * will slow down so the camera can catch up.
	 * 
	 * @param speedM
	 *            , the speed, meter/sec, of the camera.
	 */
...
<<<<<<< HEAD
				&& this.getCharacterController().getCharacter().getPosition().x
						- this.getCharacterController().getCharacter()
								.getDeahLimit() < 400) {
			this.getCharBody().applyForceToCenter(new Vector2(1, 0), true);
